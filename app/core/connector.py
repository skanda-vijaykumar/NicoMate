...
class LLMConnectorSelector:
    ...
    def __init__(self):
        # Chatmodel
        self.llm = ChatOllama(model="llama3.1", base_url="http://ollama:11434", cache=False)
        # Structure for the LLM response
        self.response_schemas = [
            ResponseSchema(name="value", description="The parsed value from user response"),
    ...
    async def parse_response_with_llm(self, response: str, question: Dict) -> Dict:
        ...
        try:
            ...
            try:
                ...
                logging.error(f"Parser error: {parse_error}. Falling back to direct parsing.")
                self.parse_failures += 1
            ...
            if self.current_question is None:
                ...
                for property_name, attr_name in property_to_attribute.items():
                    if property_name in self.answers:
                        ...
                        if confidence > 0.7:
                            logging.info(f"Skipping question about {attr_name} as it was extracted from initial message")
    ...
    def _process_parsed_requirements(self, parsed, message):
                    ...
                    if value['value'] is not None:
                        ...
                        if attr == 'wire_gauge':
                            ...
                            if awg_value is not None:
                                ...
                                logging.info(f"Extracted AWG value: {awg_value} (from {raw_value}) with confidence {confidence}")
                                self.asked_questions.add(attr)
                                ...
                                if 'connection_types' not in self.answers and 'connection_type' not in self.answers:
                                    ...
                                    self.asked_questions.add('connection_types')
                                    logging.info(f"Inferred PCB-to-Cable connection from AWG mention with confidence {confidence * 0.9}")
                            else:
                                logging.info(f"Could not normalize AWG value: {raw_value}")
        ...
        if critical_attributes_met and best_score >= 57 and (best_score - max_other_score) > 25:
            logging.info(f"Initial message provided sufficient critical information (Score: {best_score})")
            ...
            if ('straight' in message.lower() and 'pcb' in message.lower() and 
                ...
                if awg_match:
                    try:
                        ...
                        logging.info(f"Directly extracted AWG{awg_value} from initial message")
                        ...
                        if 10 <= awg_value <= 40: 
                            ...
                            for connector_name, connector_specs in self.connectors.items():
                                ...
                                if awg_value not in normalized_supported:
                                    logging.info(f"AWG{awg_value} is NOT supported by {connector_name} (supported: {normalized_supported})")
        ...
        try:
            ...
            if 'connection_types' in self.answers:
                ...
                if isinstance(connection_type, str) and connection_type.lower() in ["pcb-to-pcb", "pcb to pcb", "board to board"]:
                    ...
                    self.answers['location'] = ("internal", 0.95)
                    self.asked_questions.add('location')
                    logging.info("PCB-to-PCB detected: Automatically skipping location/panel mount question")
            ...
            if parsed_response and 'value' in parsed_response:
                ...
                for connector_name, connector_specs in self.connectors.items():
                    try:
                        ...
                        logging.error(f"Error calculating score for {connector_name}: {str(score_error)}")
                        continue
                ...
                if (best_score >= 75 and 
                    ...
                    try:
                        ...
                        logging.error(f"Error generating recommendation: {str(rec_error)}")
                ...
                if self.current_question:
                    ...
                    try:
                        ...
                        logging.error(f"Error generating final recommendation: {str(final_error)}")
                        return {
                            "status": "error",
    ...
    def calculate_connector_score(self, connector_specs: Dict, answers: Dict) -> float:
        ...
        for attr, (value, confidence) in answers.items():
            ...
            # Connection Types handling
                ...
                if user_wants_right_angle:
                    ...
                    matched_attrs.append(attr)
                    logging.info(f"Straight configuration requested - {connector_specs.get('type', 'unknown')} supports this")
            ...
            # Special handling for pin count
                ...
                if required_power and has_power:
                    attr_score = 1.5 
                    matched_attrs.append(attr)
                    logging.info(f"Connector supports high power/frequency - compatible with answer: {required_power}")
                    ...
                    logging.info(f"Connector doesn't support required high power/frequency (CRITICAL MISMATCH)")
                else:
                    attr_score = 1.0
                    matched_attrs.append(attr)
            ...
            # Special handling for pitch size
                ...
                if abs(pitch_value - spec_pitch) < 0.05:
                    attr_score = 2.0
                    matched_attrs.append(attr)
                    logging.info(f"PITCH MATCH: {connector_specs.get('type', 'unknown')} pitch {spec_pitch}mm matches requested {pitch_value}mm")
                else:
                    attr_score = 0.1
                    critical_mismatch = True
                    unmatched_attrs.append(attr)
        ...
        if matched_attrs:
            logging.info(f"Matched attributes for {connector_specs.get('type', 'unknown')}: {', '.join(matched_attrs)}")
        if unmatched_attrs:
            logging.info(f"Unmatched attributes for {connector_specs.get('type', 'unknown')}: {', '.join(unmatched_attrs)}")
        ...
        if critical_mismatch:
            ...
            logging.info(f"Critical mismatch for {connector_specs.get('type', 'unknown')}: {', '.join(critical_mismatch_factors)}")
