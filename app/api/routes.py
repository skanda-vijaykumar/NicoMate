...
async def health_check():
    ...
    return {
        ...
        "agents_available": (
            agent_queue.qsize() if hasattr(agent_queue, "qsize") else "unknown"
        ),
    }
...
async def chat(request: Request):
    ...
    try:
        ...
        if route == "selection":
            ...
            return StreamingResponse(
                generate_response(
                    user_input, formatted_chat_history, agent, session_history
                ),
                media_type="text/plain",
            )
...
async def new_session():
    ...
    if not app_ready:
        logging.info(
            "Waiting for application startup to complete before creating new session..."
        )
        if not startup_complete.wait(timeout=60):
            raise HTTPException(
                status_code=503,
...
async def get_session(session_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="An error occurred while retrieving the session."
        )
...
async def suggestion(request: Request):
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail="Error storing suggestion in database"
            )
        ...
        raise HTTPException(
            status_code=500, detail="An error occurred while submitting suggestion."
        )
...
async def get_source_document(file_path: str, page: int = 1):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error reading source document: {str(e)}"
        )
