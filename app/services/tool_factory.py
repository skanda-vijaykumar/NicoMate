    ...
    def postprocess_nodes(self, nodes, query_bundle):
        ...
        for i, node in enumerate(reranked_nodes):
            if hasattr(node, 'node') and hasattr(node.node, 'metadata'):
                ...
                logging.info(f"Node {i+1}: {source} | Family: {family} | Type: {file_type} | Score: {score}")
...
def create_tools() -> List[BaseTool]:
    ...
    try:
        ...
        logging.info(f"- vector_index_markdown: {type(vector_index_markdown).__name__ if vector_index_markdown is not None else 'None'}")
        logging.info(f"- keyword_index_markdown: {type(keyword_index_markdown).__name__ if keyword_index_markdown is not None else 'None'}")
        logging.info(f"- vector_index_markdown_lab: {type(vector_index_markdown_lab).__name__ if vector_index_markdown_lab is not None else 'None'}")
        logging.info(f"- keyword_index_markdown_lab: {type(keyword_index_markdown_lab).__name__ if keyword_index_markdown_lab is not None else 'None'}")
